...
// the player gets into range.
// it uses a simple finite state machine for the logic, which should be pretty easy to mod.
Game_CharacterBase.prototype.dashAttack=function(dashLength,leashX,leashY,battleCry,shieldDash,wallImpactAction)
{
	const maxRange = dashLength || 5;

	this._moveSpeed = 7
	this.user()._moveSpeed = 7
	this.user().shootForward(dashLength)
	this.shootForward(maxRange)
	// Need to change player speed back within move route
}
